...
## 2026-06-28 - SPA Skip Link Accessibility
...
**Action:** Use a combination of a native anchor tag and an `onClick` handler for SPA skip links, styling them with `transform: translate(-50%, -100%)` for absolute hiding and `translate(-50%, 0)` on `:focus`.
